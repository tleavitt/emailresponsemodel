...
DEFAULT_VOCAB_PATH = './data/vocab.txt'
DEFAULT_VECTOR_PATH = './data/wordVectors.txt'

def loadWordVectorsFromDisk(tokens, filepath=DEFAULT_FILE_PATH, dimensions=50):
...
# Load the first N word vectors from filepath into memory. 
# returns a dictionary of tokens and a numpy matrix of word vector values.
# each row of the matrix is one value
def loadWordVectorsIntoMemory(filepath=DEFAULT_FILE_PATH, N=50000, dimensions=50):
    """Read pretrained GloVe vectors"""
    tokens = collections.defaultdict(int)
    wordVectors = np.zeros((N, dimensions))
    wordVectors[0] = np.array([0.] * dimensions) # default value is all zeros
    i = 1
    with open(filepath) as ifs:
        for line in ifs:
            if i >= N:
            ...
            token = row[0]
            tokens[token] = i
            data = [np.float32(x) for x in row[1:]]
            if len(data) != dimensions:
                raise RuntimeError("wrong number of dimensions")
            wordVectors[i] = np.asarray(data)
            i += 1
    return tokens, wordVectors
